...
int
Voraz::get_min_node(std::map<int, float> node_distances) {
      float min = FLT_MAX;
      int idNode = -1;

      for (std::map<int, float>::const_iterator it = node_distances.begin(); it != node_distances.end(); ++it) {
            if (it->second < min) {
                  min = it->second;
                  idNode = it->first;
            }
...
void
Voraz::delete_node(std::map<int, float>& node_distances, int idNode) {
      std::map<int, float>::iterator it = node_distances.find(idNode);
...
Voraz::construction(Instancia* inst) {
      ...
      for (int i = 1; i < inst->size(); i++) {
            int idNode = initialNode; // Node Added like first, because we have to jump from this
                                      // node...
            std::map<int, float> node_distances = inst->get_column(idNode);
